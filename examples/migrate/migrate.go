...
// OldChainJSON defines the structure of the old JSON format.
...
func main() {
	jsonPath := "json" // Directory containing old .json files
	dbPath := "db"     // Path for the new Pebble database
	numWorkers := runtime.NumCPU()

	log.Printf("Starting migration with %d workers...", numWorkers)
	...
	if err != nil || len(files) == 0 {
		log.Fatalf("No files found in '%s': %v", jsonPath, err)
	}
